...
## 8. Pipeline 6 — Position Monitor

**File:** `agents/job2_position.py`  
**Model:** Claude Sonnet (`claude-sonnet-4-6`)  
**Interval:** Every `JOB2_CHECK_INTERVAL_MINUTES` (default: 30 min)

For each open MT5 position across all active pairs, Job 2 calls:

### System prompt

```
You are a professional {display} forex risk manager.
...
=== OPEN POSITION ===
  ...
  Direction    : BUY
  Volume       : 0.12 lots
  Opened       : 2026-04-08 09:17 EST (2.5h ago)
  Open price   : 1.27431
  Current price: 1.27890
  P&L          : +36.0 pips  ($43.20)
  Stop Loss    : 1.26800  (83 pips away)
  Take Profit  : 1.28200  (31 pips to go)

=== ACCOUNT STATUS ===
  [equity, balance, free margin, margin level, drawdown]

=== CURRENT MARKET SNAPSHOT ===
  [live bid/ask/spread]

=== RECENT MARKET CONTEXT ===
  [last 7 days summary + today's news]
...
Hold recommendations are posted to Slack as informational messages (no approval needed). Trim and Exit signals are saved as pending and require approval.
...
| `JOB3_RISK_PCT` | `1.0` | % of equity risked per trade |
