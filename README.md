...
You stay in control. The agent never trades without your approval. Signals arrive in Slack, you type `approve XXXXXXXX`, and the trade executes. You can also reject signals, approve at a specific limit price (`approve XXXXXXXX limit 1.0820`), start a pair-specific chat session, and have a full conversation with the agent about market conditions — all from Slack.

**Multi-agent ensemble debate.** Every Long or Short signal passes through a three-persona debate before reaching you. A Bull analyst, a Bear analyst, and a Devil's Advocate argue the trade concurrently (via `asyncio.gather`). A Judge model synthesises the debate, scores uncertainty 0–100, and can veto signals above the threshold or adjust SL/TP. The debate summary is included in every Slack signal alert. You can also trigger a separate Hold/Trim/Exit debate on your open positions at any time with `debate positions`.
...
### Signal debate (Bull / Bear / Devil's Advocate → Judge)

Three persona calls run concurrently via `asyncio.gather`:

| Persona | Role |
|---|---|
| **Bull analyst** | Argues the strongest case for entering the trade |
| **Bear analyst** | Argues the strongest case against it |
| **Devil's Advocate** | Challenges whichever side is winning — surfaces overlooked risks |

A Judge model (default: Claude Sonnet) then reads all three arguments alongside the signal's entry, SL, and TP and returns:
...
```json
{
  "evaluations": {
    ...
    "neutral_score": 58,
    "winning_case": "Bull",
    "flaws": "Ignores USD safe-haven bid from rising VIX"
  },
  "uncertainty_score": 38,
  "uncertainty_rationale": "Clear technical setup but macro headwinds present",
  "final_decision": "Long",
...
If `uncertainty_score` exceeds `DEBATE_MAX_UNCERTAINTY` (default 70), the signal is automatically downgraded to **Wait** and a veto reason is added to the Slack alert. The Judge can also adjust SL/TP and override confidence.

The debate summary appears in every signal notification:

```
Ensemble Debate — Long/Short/Wait?
📈 Bull 72 | 📉 Bear 45 | ⚖️ Neutral 58 | Winner: Bull
🟢 Uncertainty: 38/100 — Clear technical setup but macro headwinds present
"Ignores USD safe-haven bid from rising VIX"

⚖️ Judge verdict: Long [Medium]
  Entry: Entry near support — reasonable
  SL: SL slightly tight for current ATR
  TP: TP at key resistance — well-placed
🔧 Adjustments: SL -> 1.0795
```

### Position debate (Hold / Trim / Exit → Judge)

Run on demand with `debate positions` (all open positions) or `debate positions USDJPY` (one pair). Uses a separate set of personas:

| Persona | Role |
|---|---|
| **Hold analyst** | Defends staying in — thesis still intact |
| **Exit analyst** | Argues for cutting — thesis broken or risk/reward deteriorated |
| **Devil's Advocate** | Surfaces cognitive bias (sunk-cost thinking, fear of giving back gains) |

The Judge returns a Hold/Trim/Exit verdict with suggested SL/TP adjustments and, for Trim, a recommended percentage to close.

### Model configuration

```python
# config.py
DEBATE_PERSONA_MODEL = "claude-haiku-4-5-20251001"  # 3 concurrent calls — fast/cheap
DEBATE_JUDGE_MODEL   = "claude-sonnet-4-6"           # synthesis step — quality matters
USE_MULTI_AGENT_DEBATE = True
DEBATE_MAX_UNCERTAINTY = 70                          # veto threshold (0–100)
```

Upgrade `DEBATE_PERSONA_MODEL` to Sonnet or Opus for richer persona arguments. Total debate latency is approximately the time of one persona call (they run in parallel) plus one judge call — typically 20–40 seconds with Haiku personas.
...
│   ├── ensemble_agent.py      # Multi-agent debate: Bull/Bear/Devil + Judge (signal & position)
...
| `DEBATE_MAX_UNCERTAINTY` | `70` | Signals with uncertainty score above this are downgraded to Wait |
| `DEBATE_PERSONA_MODEL` | `claude-haiku-4-5-20251001` | Model for Bull/Bear/Devil's Advocate personas |
