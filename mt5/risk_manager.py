...
# EURUSD: 1 pip = 0.0001, pip value = $10 per standard lot
_PIP = 0.0001
_PIP_VALUE_PER_LOT = 10.0


def calculate_lot_size(equity: float, risk_pct: float, sl_pips: float) -> float:
    """
    ...
    raw_lot = risk_amount / (sl_pips * _PIP_VALUE_PER_LOT)
    lot = round(max(config.JOB3_MIN_LOT, min(config.JOB3_MAX_LOT, raw_lot)), 2)
    logger.info(
        f"Lot sizing: equity=${equity:.2f} risk={risk_pct}% "
...
def calculate_sl_tp(
    ...
    if sl_price is None:
        offset = config.JOB3_DEFAULT_SL_PIPS * _PIP
        sl_price = round(
            current_price - offset if direction == "buy" else current_price + offset,
            5,
        )
        logger.info(f"SL fallback: {sl_price} ({config.JOB3_DEFAULT_SL_PIPS} pips from {current_price})")

    if tp_price is None:
        offset = config.JOB3_DEFAULT_TP_PIPS * _PIP
        tp_price = round(
            current_price + offset if direction == "buy" else current_price - offset,
            5,
        )
        logger.info(f"TP fallback: {tp_price} ({config.JOB3_DEFAULT_TP_PIPS} pips from {current_price})")

    return sl_price, tp_price


def sl_pips_from_price(sl_price: float, current_price: float, direction: str) -> float:
    """Return SL distance in pips given an SL price."""
    if direction == "buy":
        return max(0.0, (current_price - sl_price) / _PIP)
    else:
        return max(0.0, (sl_price - current_price) / _PIP)
